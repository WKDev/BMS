...
class PMS7003(object):
    ...
    def __init__(self):
            #serial setting
        self.ser = serial.Serial(SERIAL_PORT, Speed, timeout = 1)

        self.prev_data = {"dust" :0.0,
                        "micro_dust": 0.0}
    ...
    def read_dust(self):
        ...
        _, data = self.get(buffer)

        if(self.protocol_chk(buffer)):
            # print("DATA read success")
            data = {"dust" :data[self.DUST_PM10_0_ATM],
                    "micro_dust": data[self.DUST_PM2_5_ATM]}
            self.prev_data = data
            return data
        else:
        ...
        if(self.protocol_size_chk(buffer)):
            ...
            if(self.header_chk(buffer)):
                ...
                if(self.chksum_chk(buffer)):
                    ...
                    print(f"[{self.__class__.__name__}]Chksum err")
            else:
                print(f"[{self.__class__.__name__}]Header err")
        else:
            print(f"[{self.__class__.__name__}]Protol err")
